...
数字会被入栈，而其它字（中字或西字）则会执行。（比如“和”）。Forth中有一些预先定义好的字可以使用，用户也可以定义自己的字。
...
“未定义！”的意思是Forth没有找到“晕”的定义内容。如果你希望让Forth能够执行它，你就必须为它进行定义。

我们可以用“令”和“毕”对一个字进行定义。“令”表示定义的开始，而“毕”则表示定义的结束。“令”后面紧跟着的就是需要定义的字，然后就是所定义的内容（直到“毕”为止）。因为这里的Forth一次只能输入一行内容，所以在使用“令”的时候，它们应该写在同一行内容里，例如：
...
当我们定义了“晕”这个字为令字以后，每次输入“晕”，都会给栈最后的值加上100。这个例子虽然简单，但它让你明白如何定义一个字。
...
“和”的意思就是将两个数相加，它从栈中取两个值，将它们相加，然后将其结果入栈。例如：
...
“差”的意思就是将两个数相减，它从栈中取两个值，将它们相减，然后将其结果入栈。例如：
...
“积”的意思就是将两个数相乘，它从栈中取两个值，将它们相乘，然后将其结果入栈。例如：
...
“商”的意思就是将两个数相除，它从栈中取两个值，将它们相除，然后将其结果入栈。例如：
...
“余”的意思就是将两个数相除，然后获取它的余数。它从栈中取两个值，将它们相除，然后将余数入栈。例如：
...
“负”的意思就是数前面加一个“-”符号，如果它是正数，它将变为负数；反之如果它是一个负数，它将变为一个正数。例如：
...
“增”的意思就是将数一，它从栈中取一个值，将它加上一，然后将结果入栈。例如：
...
“减”的意思就是将数减一，它从栈中取一个值，将它减去一，然后将结果入栈。例如：
...
“倍”的意思就是将数翻倍，它从栈中取一个值，将它乘以2，然后将结果入栈。例如：
...
“衰”的意思就是将数减倍，它从栈中取一个值，将它除以2，然后将结果入栈。例如：
...
“方”的意思就是计算一个数的平方，它从栈中取一个值，将它乘以它自己，然后将结果入栈。例如：
...
“根”的意思就是计算一个数的平方根，它从栈中取一个值，计算它的平方根，然后将结果入栈。例如：
...
“幂”的意思就是计算两个数的幂，即n<sup>m</sup>，它从栈中取两个值，分别作为底数和指数，对其进行计算，然后将结果入栈。例如：
...
有些字不对数本身做任何运算，然而它们仍然会对栈中的值作出改变。
...
最简单的显示方法就是“印”，你可以通过它出栈一个数，并将其显示在终端上。试着输入下面的内容：
...
不像“印”，“字”显示的内容后面不会增加任何东西，这让你能够完整的显示一句话。

### 行(|)

“行”是另起一行的意思，它让显示的位置移动到新的一行：
